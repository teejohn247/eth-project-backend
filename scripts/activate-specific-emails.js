...
// Target emails to activate
const TARGET_EMAILS = [
  'djsmokevibez042@gmail.com',
  'donatusosagie762@gmail.com', 
  'iyorebinah@gmail.com',
  '85palmerefosa@gmail.com'
];
...
// Function to process specific emails
const processSpecificEmails = async () => {
  try {
    console.log('🔍 Searching for specific email accounts...');
    console.log('Target emails:', TARGET_EMAILS.join(', '));
    
    // Find users with the specific emails
    const users = await User.find({ 
      email: { $in: TARGET_EMAILS }
    }).select('+password');
    
    console.log(`📊 Found ${users.length} matching accounts out of ${TARGET_EMAILS.length} requested`);
    ...
    // Show found accounts and their status
    console.log('\n📋 Account status:');
    const foundEmails = users.map(u => u.email);
    const notFoundEmails = TARGET_EMAILS.filter(email => !foundEmails.includes(email));
    ...
    users.forEach((user, index) => {
      ...
      if (issues.length > 0) {
        ...
        console.log(`   ✅ Account already active and complete`);
      }
    });
    ...
    // Filter users that need activation
    const usersNeedingActivation = users.filter(user => 
      !user.isEmailVerified || !user.isPasswordSet || !user.isActive
    );
    
    if (usersNeedingActivation.length === 0) {
      console.log('\n✅ All found accounts are already fully activated');
      return;
    }
    
    console.log(`\n🔧 ${usersNeedingActivation.length} accounts need activation`);
    ...
    for (const user of usersNeedingActivation) {
      ...
      activationResults.push({ user, isNewlyVerified, changes });
      console.log(`✅ ${user.firstName} ${user.lastName} (${user.email}) - ${changes.join(', ')}`);
    }
    ...
    const emailPromises = activationResults.map(async ({ user, isNewlyVerified }) => {
      try {
    ...
    console.log('\n📊 Final Summary:');
    console.log(`👥 Total target emails: ${TARGET_EMAILS.length}`);
    console.log(`📁 Found in database: ${users.length}`);
    console.log(`🔧 Needed activation: ${usersNeedingActivation.length}`);
    console.log(`✅ Emails sent successfully: ${successfulEmails}`);
    console.log(`❌ Email sending failed: ${failedEmails}`);
    ...
    console.error('❌ Error during specific email processing:', error);
  }
};
...
const main = async () => {
  ...
  console.log('🎭 Edo Talent Hunt - Specific Email Activation');
  console.log('===========================================\n');
