...
class EmailService {
  ...
  async sendTicketEmail(
    ...
    tickets: Array<{ ticketType: string; quantity: number; unitPrice: number; totalPrice: number }>,
    ticketNumbers: string[],
    totalAmount: number
  ): Promise<void> {
    const subject = '🎟️ Your Edo Talent Hunt Tickets';
    const htmlContent = this.generateTicketEmailTemplate(
      firstName,
      lastName,
      purchaseReference,
      tickets,
    ...
    const { TicketPdfGenerator } = await import('../utils/ticketPdfGenerator');
  ...
  private generateTicketEmailTemplate(
    ...
    // Logo uploaded to Cloudinary for reliable email display
